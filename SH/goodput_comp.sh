...
# params 1 MINUSERS, 2 MAXUSERS, 3 INTERVAL, 4 FILENAME, 5 FILE, 6 NUMSIM, 7 FLOW-TYPE, 8 FILE_NAME_PLOT,
...
# simulation time for a simulation round, defined in MultiCellWithBorderPicos.h
time=100
...
HOTYPE3=3       # Stands for the Proposed HO 
	...
	COUNT=1
        ...
        until [ $NBUE -gt $2 ]; do
                ...
                grep "RX "$7   $TOTALNAME3  | awk '$14<0.1 {print $8+5}'  > tmp
                ./compute_throughput.sh tmp >> temporal
                rm tmp
                 let COUNT=COUNT+1
                done
                ./compute_average.sh temporal | awk '{print "'$NBUE' "$1}' >> 3kmph_HOType1_goodput_$8_$7.xlsx
                rm temporal
        let NBUE=NBUE+$3
        done
        ...
        until [ $NBUE -gt $2 ]; do
                ...
                grep "RX "$7   $TOTALNAME30  | awk '$14<0.1 {print $8+5}'  > tmp
                ./compute_throughput.sh tmp >> temporal
                rm tmp
                 let COUNT=COUNT+1
                done
                ./compute_average.sh temporal | awk '{print "'$NBUE' "$1}' >> 30kmph_HOType1_goodput_$8_$7.xlsx
                rm temporal
        let NBUE=NBUE+$3
        done
        ...
        until [ $NBUE -gt $2 ]; do
                ...
                grep "RX "$7   $TOTALNAME120  | awk '$14<0.1 {print $8+5}'  > tmp
                ./compute_throughput.sh tmp >> temporal
                rm tmp
                 let COUNT=COUNT+1
                done
                ./compute_average.sh temporal | awk '{print "'$NBUE' "$1}' >> 120kmph_HOType1_goodput_$8_$7.xlsx
                rm temporal
        let NBUE=NBUE+$3
        done
        ...
        until [ $NBUE -gt $2 ]; do
                ...
                grep "RX "$7   $TOTALNAME200  | awk '$14<0.1 {print $8+5}'  > tmp
                ./compute_throughput.sh tmp >> temporal
                rm tmp
                 let COUNT=COUNT+1
                done
                ./compute_average.sh temporal | awk '{print "'$NBUE' "$1}' >> 200kmph_HOType1_goodput_$8_$7.xlsx
                rm temporal
        let NBUE=NBUE+$3
        done

        # GRAPHIC FOR 300kmph of the standard Hard HO
        COUNT=1
        NBUE=$1
        until [ $NBUE -gt $2 ]; do
                COUNT=1
                ...
                grep "RX "$7   $TOTALNAME300  | awk '$14<0.1 {print $8+5}'  > tmp
                ./compute_throughput.sh tmp >> temporal
                rm tmp
                 let COUNT=COUNT+1
                done
                ./compute_average.sh temporal | awk '{print "'$NBUE' "$1}' >> 300kmph_HOType1_goodput_$8_$7.xlsx
                rm temporal
        let NBUE=NBUE+$3
        done
...
#System-Throughput Users-per-Cell System-Throughput[Mbps]


echo  System-thoughput $7 REPORT FINISHED!! 

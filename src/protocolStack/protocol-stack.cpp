...
ProtocolStack::ProtocolStack(NetworkNode* device)
{
  ...
  if (device->GetNodeType() == NetworkNode::TYPE_ENODEB
		  || device->GetNodeType() == NetworkNode::TYPE_PICO_NODE)  // added, 20141223 P-UE uses the same MacEntity as M-UE
    {
      m_mac = new EnbMacEntity ();
      m_mac->SetDevice(device);
    }
  else if (device->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION) // MacEntity for H-UE is comprehensive, since only CSG user can access HeNB
  {

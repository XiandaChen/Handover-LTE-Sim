...
#include "scenarios/macro-with-pico.h"
  ...
  if (argc > 1)
    {
	  ...
	  // added, 20141223
	  if (strcmp(argv[1], "MacroWithPico")==0)
	  {
		  double cellRadius = atof(argv[2]);
		  int nbCells = atoi(argv[3]);
		  int nbRNs = atoi(argv[4]);
		  int relay_ptx = atoi(argv[5]);
		  double relay_position = atof(argv[6]);
		  int nbUEs = atoi(argv[7]);
		  int nbVoip = atoi(argv[8]);
		  int nbVideo = atoi(argv[9]);
		  int nbBe = atoi(argv[10]);
		  int videoBitRate = atoi(argv[11]);
		  int format_video = atoi(argv[12]);
		  int sched_type = atoi(argv[13]);
		  double MAX_DELAY = atof(argv[14]);
		  MacroWithPico(cellRadius, nbCells, nbRNs, relay_ptx, relay_position, nbUEs, nbVoip, nbVideo, nbBe, videoBitRate, format_video, sched_type, MAX_DELAY);
	  }

...
#include "../utility/eesm-effective-sinr.h"		// added, 20150106, cout the SINR when trigger ho
...
NetworkManager::UpdateUserPosition (double time)
{
  ...
  for (iter = records->begin(); iter != records->end(); iter++)
    ...
    std::cout << "\t USER  " << record->GetIDNetworkNode ()
      ...
      record->UpdateUserPosition (time);		// added, 20150102, comment: UE position update
...
NetworkManager::CheckHandoverPermissions (NetworkNode* target, UserEquipment* user)
{
	if ( target->GetNodeType() == NetworkNode::TYPE_ENODEB
			|| target->GetNodeType() == NetworkNode::TYPE_PICO_NODE )  // added, 20141223
	{
		return true;
	}
...
NetworkManager::TransferBearerInfo (UserEquipment* ue, NetworkNode* target)
{
  ...
  if ( (target->GetNodeType() == NetworkNode::TYPE_ENODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB) ||
		 ...
		 (target->GetNodeType() == NetworkNode::TYPE_PICO_NODE && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_ENODEB) ||
		 (target->GetNodeType() == NetworkNode::TYPE_ENODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_PICO_NODE) ||
		 (target->GetNodeType() == NetworkNode::TYPE_PICO_NODE && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION) ||
		 (target->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_PICO_NODE))
    {
	   ...
	   //added, 20141230, to add message showing handover time and location
	 #ifdef HANDOVER_DEBUG
	   std::cout << "\nSUCCESSFUL_HANDOVER for user " << ue->GetIDNetworkNode()
	 		    << " AT_THE_MOMENT " << Simulator::Init()->Now()
	 		    << " from CELL_" << oldTargetNode->GetIDNetworkNode()
	 		    << " to CELL_" << newTargetNode->GetIDNetworkNode() << "\n" << std::endl;
	 #endif
    }
}
